...
import { sendConsultationEmail, sendEmailFallback } from '../lib/email'
import {
  MessageCircle,
  Calendar,
  FileText,
...
export const SmartScheduler = () => {
  ...
  const handleScheduleSubmit = async () => {
    ...
    try {
      ...
      setIsSubmitting(false)
      setSubmitError('Unable to send email automatically. Please try again or contact us directly.')
      
      // As fallback, show the error for 3 seconds then offer manual email option
      setTimeout(() => {
        const fallbackConfirm = window.confirm(
          'Would you like to open your email client to send the consultation request manually?'
        )
        if (fallbackConfirm) {
          window.location.href = sendEmailFallback(formData)
        }
        setSubmitError('')
      }, 3000)
    }
    ...
    >
      ...
      <div className="space-y-4">
        ...
        <motion.button
          ...
          className={`w-full p-3 rounded-md font-semibold transition-all duration-300 transform hover:scale-105 hover:-translate-y-0.5 active:scale-95 shadow-token-sm hover:shadow-medium flex items-center justify-center gap-2 ${
            isSubmitted
...
export const LegalCaseStrategySimulator = () => {
  ...
  const downloadCaseAnalysis = () => {
    ...
    doc.setFontSize(20)
    doc.setFont('helvetica', 'bold')
    doc.text('LEGAL CASE STRATEGY ANALYSIS', 105, 25, { align: 'center' })

    doc.setFontSize(12)
    doc.setFont('helvetica', 'normal')
    doc.text('Generated by Tim Harmar Legal - Case Strategy Simulator', 105, 35, {
      align: 'center',
    })
    ...
    const disclaimerText =
      'This analysis provides general information only and does not constitute legal advice. For personalized legal guidance, schedule a consultation with Tim Harmar Legal.'
    ...
    doc.setFontSize(12)
    doc.setFont('helvetica', 'bold')
    doc.text('CASE ANALYSIS DETAILS', 20, currentY)
    currentY += 10
    ...
    const details = [
      `Case Type: ${caseTypes[analysisResults.caseType]?.name || 'N/A'}`,
    ...
    // Add potential causes of action
    if (analysisResults.causesOfAction && analysisResults.causesOfAction.length > 0) {
      doc.setFont('helvetica', 'bold')
      doc.text('POTENTIAL CAUSES OF ACTION:', 20, currentY)
      currentY += 7

      doc.setFont('helvetica', 'normal')
      analysisResults.causesOfAction.forEach((cause, i) => {
        doc.text(`${i + 1}. ${cause.name}`, 20, currentY)
        currentY += 5
        const description = doc.splitTextToSize(`   Description: ${cause.description}`, 170)
        doc.text(description, 20, currentY)
        currentY += description.length * 5 + 3

        doc.text('   Required Elements:', 20, currentY)
        currentY += 5
        cause.elements.forEach((element) => {
          const elementText = doc.splitTextToSize(`   • ${element}`, 170)
          doc.text(elementText, 20, currentY)
          currentY += elementText.length * 5
        })

        currentY += 3
        doc.text('   Available Remedies:', 20, currentY)
        currentY += 5
        cause.remedies.forEach((remedy) => {
          const remedyText = doc.splitTextToSize(`   • ${remedy}`, 170)
          doc.text(remedyText, 20, currentY)
          currentY += remedyText.length * 5
        })
        currentY += 5
      })
    }

    // Add procedural aspects
    if (analysisResults.proceduralAspects && analysisResults.proceduralAspects.length > 0) {
      doc.setFont('helvetica', 'bold')
      doc.text('PROCEDURAL ASPECTS:', 20, currentY)
      currentY += 7

      doc.setFont('helvetica', 'normal')
      analysisResults.proceduralAspects.forEach((aspect, i) => {
        const aspectText = doc.splitTextToSize(`${i + 1}. ${aspect}`, 170)
        doc.text(aspectText, 20, currentY)
        currentY += aspectText.length * 5 + 3
      })
      currentY += 5
    }

    // Add recommended approach
    if (analysisResults.recommendedApproach && analysisResults.recommendedApproach.length > 0) {
      doc.setFont('helvetica', 'bold')
      doc.text('RECOMMENDED APPROACH:', 20, currentY)
      currentY += 7

      doc.setFont('helvetica', 'normal')
      analysisResults.recommendedApproach.forEach((step, i) => {
        const stepText = doc.splitTextToSize(`${i + 1}. ${step}`, 170)
        doc.text(stepText, 20, currentY)
    ...
    if (analysisResults.alternativeOptions && analysisResults.alternativeOptions.length > 0) {
      doc.setFont('helvetica', 'bold')
      doc.text('ALTERNATIVE OPTIONS:', 20, currentY)
      currentY += 7

      doc.setFont('helvetica', 'normal')
      analysisResults.alternativeOptions.forEach((option) => {
        const optionText = doc.splitTextToSize(`• ${option}`, 170)
        doc.text(optionText, 20, currentY)
        currentY += optionText.length * 5 + 3
      })
      currentY += 5
    }
    ...
    currentY += 10
    doc.setFont('helvetica', 'bold')
    doc.text('CONTACT TIM HARMAR LEGAL:', 20, currentY)
    currentY += 7
    ...
    doc.text(`© 2025 Tim Harmar: Legal and Consulting Services`, 105, 285, { align: 'center' })

    // Save the PDF
    doc.save(`case-strategy-analysis-${Date.now()}.pdf`)
  }
